...
function ArticleFooter({ classes, article, subscribe }) {
      ...
      <Col xs={12} sm={12} md={8} lg={8}>
        <p className={classes.inPrint}>
          ...
          {section.permalink.includes('special-issues') ? (
            <span>
              <Link to={section.permalink}>{section.name}</Link>
              ,
              {' '}
              {section.description}
            </span>
        ...
        <p className={classes.subscriptionCTA}>
          ...
          <i>Spectator</i>
          {' '}
          issue again. Sign up for our newsletter
...
const mapDispatchToProps = (dispatch, ownProps) => bindActionCreators({ subscribe }, dispatch);
